    ...
    <section class="panel panel-featured panel-featured-success">
        <header class="panel-heading">
            <div class="row">
                ...
                <a href="./index.php?page=Iconnet&tahun=true"><button type="button" class="mb-xs mt-xs mr-xs btn btn-primary">Tahun Ini</button></a>
                <a href="./index.php?page=Iconnet"><button type="button" class="mb-xs mt-xs mr-xs btn btn-danger">Tagihan Aktif</button></a>
             
                </div>
                <DIV class="col-md-6">
                <?php if($_SESSION['type'] != "Staff"): ?>
                <div class="col-md-6 col-md-offset-6">
                <a class="btn btn-block btn-sm btn-default btn-flat border-primary" href="./index.php?page=iconnet_add"><i class="fa fa-plus"></i> Tambah Iconnet</a>
                </div>
                <?php endif; ?>
                </div>
            </div>
            ...
            <div class="table-responsive">
                <table class="table table-hover mb-none" id="datatable-default">
					...
					<tr>
                        ...
                        <th class="text-center">Jumlah PA</th>
                    ...
                    if(isset($_GET['tahun'])){
                        $qry = $conn->query("SELECT * FROM tbl_iconnet where periode like concat('%',year(now())) order by id desc");
                    } else {
					    $qry = $conn->query("SELECT * FROM tbl_iconnet where payment < 2 order by id desc");
                    }
					...
					<tr id="<?php echo $row['id'] ?>">
						...
						<td class="center"><?php echo $row['sektor'] ?></td>
						...
						<td class="center actions-hover actions-fade">
                            <a href="./index.php?page=iconnet_detail&pid=<?php echo $row['id'] ?>"><i class="fa fa-search"></i></a>
                            <a href="./index.php?page=iconnet_edit&id=<?php echo $row['id'] ?>"><i class="fa fa-edit"></i></a>
                            <a href="./iconnet_del.php?id=<?php echo $row['id'] ?>&periode=<?php echo $row['periode'] ?>" onclick="return confirm('Are you sure want to delete this project?')"><i class="fa fa-trash-o remove"></i></a>
                        </td>
					</tr>	
				<?php endwhile; ?>
				</tbody>
